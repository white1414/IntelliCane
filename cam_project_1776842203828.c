...
// v2.4: widened click windows because users couldn't reliably hit 4
// clicks within the old 500/600 ms windows. Now any press up to 800 ms
// counts as a click and gaps up to 700 ms keep the click sequence
// alive, so a deliberate "tap-tap-tap-tap" comfortably groups.
#define CLICK_MAX_PRESS_MS      800   // press shorter than this counts as a click
#define CLICK_GROUP_GAP_MS      700   // gap between clicks to be considered same group
...
static void sos_button_task(void *arg) {
    ...
    for (;;) {
        ...
        // 10 ms poll: catches faster button presses than the old 20 ms
        // loop. Important now that 4-click toggle is a real user flow.
